...
function Checkout() {
  ...
  useEffect(() => {
    ...
    const fetchRoomDetails = async () => {
      try {
        const response = await fetch(`https://andriyputiyk-001-site1.htempurl.comapi/Room/${roomId}`);
        if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://andriyputiyk-001-site1.htempurl.comapi/Booking', {
        method: 'POST',

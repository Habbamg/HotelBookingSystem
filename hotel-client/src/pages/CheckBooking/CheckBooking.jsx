...
const CheckBooking = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/Booking/check-status?phone=${encodeURIComponent(phone.trim())}&bookingCode=${encodeURIComponent(bookingCode.trim())}`);
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/Booking/cancel?bookingCode=${encodeURIComponent(booking.bookingCode)}`, {
        method: 'POST'
      });

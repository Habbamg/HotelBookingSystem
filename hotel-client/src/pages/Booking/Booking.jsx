...
function Booking() {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        setLoading(true);
        let url = 'https://andriyputiyk-001-site1.htempurl.comapi/Room'; 

        if (urlStart && urlEnd) {
          url = `https://andriyputiyk-001-site1.htempurl.comapi/Room/search?checkIn=${urlStart}&checkOut=${urlEnd}&adults=${urlAdults}&children=${urlChildren}`;
        }

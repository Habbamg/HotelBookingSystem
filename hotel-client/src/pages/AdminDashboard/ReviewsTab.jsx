...
function ReviewsTab() {
  ...
  const fetchReviews = async () => {
    try {
      ...
      const response = await fetch('https://andriyputiyk-001-site1.htempurl.comapi/Review/all', {
  ...
  const handleApprove = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://andriyputiyk-001-site1.htempurl.comapi/Review/${id}/approve`, {
        method: 'PUT',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://andriyputiyk-001-site1.htempurl.comapi/Review/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (response.ok) {

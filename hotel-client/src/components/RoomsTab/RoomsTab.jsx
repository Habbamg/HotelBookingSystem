...
const RoomsTab = () => {
  ...
  const handleSaveFullRoom = async () => {
    try {
      ...
      if (editingRoomId) {
        // Оновлюємо існуючий
        const res = await fetch(`/api/Room/${editingRoomId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
      ...
      for (const amenityId of amenitiesToAdd) {
        await fetch(`/api/Room/${roomId}/amenity/${amenityId}`, {
     ...
     // --- ОНОВЛЕННЯ ФОТОГРАФІЙ ---
      ...
      if (editingRoomId) {
        ...
        await fetch(`/api/Room/${roomId}/sync-images`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        ...
        for (const url of validImages) {
          await fetch(`/api/Room/${roomId}/media`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },

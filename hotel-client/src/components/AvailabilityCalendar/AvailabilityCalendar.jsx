...
const AvailabilityCalendar = () => {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const roomsRes = await fetch('https://andriyputiyk-001-site1.htempurl.comapi/Room'); 
      if (roomsRes.ok) setRooms(await roomsRes.json());

      if (startDate && endDate) {
        const startStr = format(startDate, 'yyyy-MM-dd');
        const endStr = format(endDate, 'yyyy-MM-dd');
        const availRes = await fetch(`https://andriyputiyk-001-site1.htempurl.comapi/Availability?start=${startStr}&end=${endStr}`);
        if (availRes.ok) setAvailabilities(await availRes.json());
      }

      const rulesRes = await fetch('https://andriyputiyk-001-site1.htempurl.comapi/Availability/occupancy-rules');
      if (rulesRes.ok) setOccupancyRules(await rulesRes.json());

      // 🔥 НОВЕ: Завантажуємо реальні бронювання
      const bookingsRes = await fetch('https://andriyputiyk-001-site1.htempurl.comapi/Booking/all', {
  ...
  const handleSaveBulkEdit = async () => {
    try {
      ...
      const response = await fetch('https://andriyputiyk-001-site1.htempurl.comapi/Availability/bulk-update', {
  ...
  const handleSaveRateRule = async () => {
    ...
    try {
      const promises = Object.keys(discountInputs).map(guestCountStr => {
        ...
        return fetch('https://andriyputiyk-001-site1.htempurl.comapi/Availability/occupancy-rule', {
          method: 'POST', 
  ...
  const handleChangeBaseCapacity = async () => {
    ...
    if (newCapacity !== null && newCapacity.trim() !== "") {
      try {
        const response = await fetch('https://andriyputiyk-001-site1.htempurl.comapi/Room/update-capacity', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },

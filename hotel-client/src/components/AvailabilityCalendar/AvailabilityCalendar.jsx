...
const AvailabilityCalendar = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (startDate && endDate) {
        ...
        const availRes = await fetch(`/api/Availability?start=${startStr}&end=${endStr}`);
        if (availRes.ok) setAvailabilities(await availRes.json());
      }

      const rulesRes = await fetch('/api/Availability/occupancy-rules');
  ...
  const handleSaveBulkEdit = async () => {
    try {
      ...
      const response = await fetch('/api/Availability/bulk-update', {
  ...
  const handleSaveRateRule = async () => {
    ...
    try {
      const promises = Object.keys(discountInputs).map(guestCountStr => {
        ...
        return fetch('/api/Availability/occupancy-rule', {
          method: 'POST', 
  ...
  const handleChangeBaseCapacity = async () => {
    ...
    if (newCapacity !== null && newCapacity.trim() !== "") {
      try {
        const response = await fetch('/api/Room/update-capacity', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
